...
function file_upload($file, $type = 'image', $name = '', $compress = false) {
	...
	switch ($type) {
		...
		case 'audio':
			...
			break;
		case 'video':
			$allowExt = array('rm', 'rmvb', 'wmv', 'avi', 'mpg', 'mpeg', 'mp4','mp4');
			$limit = $setting['upload']['audio']['limit'];
			break;
	}
	...
	//file_put_contents(IA_ROOT . "/addons/hc_doudou/test11.txt", date('Y-m-d H:i:s').'game :'.$ext.'game :'.var_export($allowExt,true).PHP_EOL, FILE_APPEND);
