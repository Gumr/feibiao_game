...
(function () {
    // 域名
    // var host = 'https://www.sanchunh.cn/app/index.php';
    var host = 'https://sport.heidoupay.com/app/index.php';
    //canvas宽度
    ...
    var CircleCenterSplit = [
        {"left":{"url":"../addons/hc_doudou/template/mobile/img/CircleCenter_1_split_left.png","w":"238","h":"407","rs":Math.PI*2},
         "right":{"url":"../addons/hc_doudou/template/mobile/img/CircleCenter_1_split_right.png","w":"230","h":"407","cutx":"26","rs":Math.PI*2}},
        {"left":{"url":"../addons/hc_doudou/template/mobile/img/CircleCenter_2_split_left.png","w":"238","h":"407","rs":Math.PI*2},
         "right":{"url":"../addons/hc_doudou/template/mobile/img/CircleCenter_2_split_right.png","w":"230","h":"407","cutx":"26","rs":Math.PI*2}},
        {"left":{"url":"../addons/hc_doudou/template/mobile/img/CircleCenter_3_split_left.png","w":"238","h":"407","rs":Math.PI*2},
         "right":{"url":"../addons/hc_doudou/template/mobile/img/CircleCenter_3_split_right.png","w":"230","h":"407","cutx":"26","rs":Math.PI*2}},
    ]
    //弹夹剩余子弹数
    ...
    //口红资源数组
    var arrayLipstick = ["../addons/hc_doudou/template/mobile/img/Lipstick_1.png","../addons/hc_doudou/template/mobile/img/Lipstick_2.png","../addons/hc_doudou/template/mobile/img/Lipstick_3.png"];
    //中心圆资源数组
    var arrayCenterCircle = ["../addons/hc_doudou/template/mobile/img/CircleCenter_1.png","../addons/hc_doudou/template/mobile/img/CircleCenter_2.png","../addons/hc_doudou/template/mobile/img/CircleCenter_3.png"];
    //关卡资源
    var levelUrl = [
        {"normal":"../addons/hc_doudou/template/mobile/img/level_icon_1_active.png","active":"../addons/hc_doudou/template/mobile/img/level_icon_1_active.png"},
        {"normal":"../addons/hc_doudou/template/mobile/img/level_icon_2.png","active":"../addons/hc_doudou/template/mobile/img/level_icon_2_active.png"},
        {"normal":"../addons/hc_doudou/template/mobile/img/level_icon_3.png","active":"../addons/hc_doudou/template/mobile/img/level_icon_3_active.png"}
    ]
...
/*        context.font = CircleCenterText+" PingFangSC-Semibold Arial"+ "border";
        ...
        if(this.restNum>=10){
            ...
            $("#bulletsNum1").attr("src","../addons/hc_doudou/template/mobile/img/"+Math.floor(this.restNum/10)+".png")
            $("#bulletsNum2").attr("src","../addons/hc_doudou/template/mobile/img/"+this.restNum%10+".png")

        }else{
            $("#bulletsNum2").attr("src","")
            $("#bulletsNum2").css("display","none")
            $("#bulletsNum1").attr("src","../addons/hc_doudou/template/mobile/img/"+this.restNum+".png")
        }
    };
    ...
    Game.prototype = {
        ...
        init:function (){
            var that = this;
            if(getParameter('level')){
                that.levelA = parseInt(getParameter('level')-1);
            }
            ...
            ballimg.src = '../addons/hc_doudou/template/mobile/img/CircleCenter_1.png';
            if(getParameter('level')){
                ballimg.src = '../addons/hc_doudou/template/mobile/img/CircleCenter_'+ getParameter('level') +'.png';
                $("#levelSwitchBoxMain").attr("src", "../addons/hc_doudou/template/mobile/img/level_"+ getParameter('level') +"_mains.jpg");
            }
            ...
            // 请求个人信息昵称头像
            $.post( host + '?i=4&t=0&v=1.2.4&from=wxapp&c=entry&a=wxapp&do=usertradeno&m=hc_doudou', {'trade_no':orderId},function(res){
                if(JSON.parse(res).errno == 0){
                    sessionStorage.setItem('userinfo',JSON.stringify(JSON.parse(res).data))
                }
            })
            // 根据用户中奖概率 控制转速
            $.post( host + '?i=4&t=0&v=1.2.4&from=wxapp&c=entry&a=wxapp&do=gamecate&m=hc_doudou', {'uid':openid},function(res){
                if(JSON.parse(res).errno == 0){
                    if(JSON.parse(res).data.rate>=0 && JSON.parse(res).data.rate<=0.25){
                        D_Json.Level3_PARAMETERS.ROTAION_SPEED_ARRAY = [-0.09,-0.07,-0.05,0.06,0.08,0.09]
                    }else if(JSON.parse(res).data.rate>0.25 && JSON.parse(res).data.rate<=0.5){
                        D_Json.Level3_PARAMETERS.ROTAION_SPEED_ARRAY = [-0.12,-0.1,-0.08,0.09,0.11,0.12]
                    }else if(JSON.parse(res).data.rate>0.5 && JSON.parse(res).data.rate<=0.75){
                        D_Json.Level3_PARAMETERS.ROTAION_SPEED_ARRAY = [-0.15,-0.13,-0.11,0.12,0.14,0.15]
                    }else if(JSON.parse(res).data.rate>0.75 && JSON.parse(res).data.rate<=1){
                        D_Json.Level3_PARAMETERS.ROTAION_SPEED_ARRAY = [-0.18,-0.16,-0.14,0.15,0.17,0.18]
                    }
                }
            })
        },
                ...
                if(!this.isCanTap){
                    ...
                    if(tempBC.y <=  ROTATION_CENTER.y + ROTATION_RADIUS + 2 * RADIUS_SMALL){
                        ...
                        if(checkCollisionResult.isCollision){
                            ...
                            $(".bulletsNumBox").css("display","none")
                            // this.resetCircleRotationEnd(context);
                            $('#gameResurgence .head').css('background-image','url('+ JSON.parse(sessionStorage.getItem('userinfo')).avatar +')')
                            $('#gameResurgence .nick').text(JSON.parse(sessionStorage.getItem('userinfo')).nickname)
                            $('#gameResurgence #gameOverBoxBtn i').text(JSON.parse(sessionStorage.getItem('userinfo')).price)
                            $('#gameResurgence .fail-level span').text(toChinesNum(that.level))
                            $("#gameResurgence").css("display","block");
                            $("#app").addClass("blur");
                            $('#gameResurgence #gameOverBoxBtn').on('click',function(){
                                $('.isAlive .down i').text(JSON.parse(sessionStorage.getItem('userinfo')).price)
                                $(".isAlive").addClass("fadelogIn");
                            })
                            $('.isAlive .take').on('click',function(){
                                $(".isAlive").removeClass("fadelogIn");
                                var params = {};
                                params.uid = openid;
                                params.trade_no = orderId;
                                $.post( host + '?i=4&t=0&v=1.2.4&from=wxapp&c=entry&a=wxapp&do=immedplay&m=hc_doudou', params,function(res){
                                    if(res.errno == '1'){
                                        alert(res.message)
                                    }else if(res.errno == '0'){
                                        $("#app").removeClass("blur");
                                        $("#gameResurgence").css("display","none");
                                        getParameter('level') ? replaceParamVal("level",that.level) : window.location.replace (location.href + '&level=' + that.level)
                                    }
                                },'json')
                            })
                            ...
                            $('#gameResurgence .jump').on('click',function(){
                                $(".isAlive").removeClass("fadelogIn");
                                $("#gameResurgence").css("display","none");
                                $('#freeAlive .fail-level span').text(toChinesNum(that.level))
                                $('#freeAlive .head').css('background-image','url('+ JSON.parse(sessionStorage.getItem('userinfo')).avatar +')')
                                $('#freeAlive .nick').text(JSON.parse(sessionStorage.getItem('userinfo')).nickname)
                                $("#freeAlive").css("display","block");
                                $('#freeAlive #gameOverBoxBtn').on('click',function(){
                                    wx.miniProgram.redirectTo({
                                        url: '../ad/ad?orderId='+orderId + '&userId=' + openid + '&level=' + that.level
                                    });
                                })
                                $("#freeAlive .jump").on("click",function(){
                                    if (window.isH5) {
                                        window.history.go(-1);
                                    }else{
        ...
        normalResults:function(){
            if(this.bulletCircles[BulletCirclesIndex]){
                this.bulletCircles[BulletCirclesIndex].imgObj.src = "../addons/hc_doudou/template/mobile/img/Sword_small_"+this.level+"_gray.png";
                BulletCirclesIndex++;
        ...
        nextLevel:function (){
           ...
           self.alive == true ? self.level : self.level++;
           self.alive = false;
           console.log('812关卡'+self.level)
        //    self.level > 3 ? self.level-- : self.level //bug
            // console.log(22,self.level,self.levelLength);
           if (self.level <= self.levelLength) {
               // console.log(11,self.level,self.levelLength);
               $("#levelSwitchBoxMain").attr("src", "../addons/hc_doudou/template/mobile/img/level_"+self.level+"_mains.jpg");
        ...
        levelChange:function (){
                    ...
                    $('.isAlive .take').on('click',function(){
                        ...
                        $.post( host + '?i=4&t=0&v=1.2.4&from=wxapp&c=entry&a=wxapp&do=immedplay&m=hc_doudou', params,function(res){
                            if(res.errno == '1'){
                                ...
                                // $("#gameResurgence").css("display","none");
                                // $("#app").removeClass("blur");
                                // if(arrayCenterCircle[that.levelA]){
                                //     that.centerCircle.imgObj.src = arrayCenterCircle[that.levelA]
                                //     //this.centerCircle.restNum = this.levelArray[this.levelA+1][1];
                                //     that.centerCircle.restNum = that.getParams(that.levelA).levelArray[1];
                                // }
                                // //插入
                                // that.centerCircle.isSplit = true;
                                // that.audioPlay($("#split_audio").get(0));
                                // // console.log("成功")
                                // for(var i=0;i<that.rotationCircles.length;i++){
                                //     that.rotationCircles[i].isSplit = true;
                                // }
                                // for(var key in that.AimBullet){
                                //     delete that.AimBullet[key];
                                // }
                                $("#app").removeClass("blur");
                                $("#gameResurgence").css("display","none");
                                getParameter('level') ? replaceParamVal("level",that.level) : window.location.replace (location.href + '&level=' + that.level)
            ...
            for(i = 0; i < lens; i++){
                ...
                tempClipImg.src = "../addons/hc_doudou/template/mobile/img/Sword_small_"+this.level+".png";
