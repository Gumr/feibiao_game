...
class step{
    ...
    public function change_currency($member_id, $currency, $type = 1, $remarks = '')
    {
        ...
        if (in_array($type, array(1, 2, 3, 4, 6, 7, 8, 10,11))) {
     ...
     * @return bool
     */
    public function calculation_currency($step, $isvip = 0)
    {
        if ($isvip == 1) {
            $step_currency = getConfig('vip_echange_currency');
        }
        else
    ...
    public function update_currency($member_id, $step_number)
    {
        ...
        $member = p_get('member', array('member_id' => $member_id));
        ...
        try {
            ...
            if (empty($row)) {
                ...
                $currency = $this->calculation_currency($update_upper,$member['isvip']);
    ...
    public function currency_log($member_id, $p, $page_size = 10)
    {
        ...
        if ($log) {
            foreach ($log as & $value) {
                ...
                $value['mold'] = in_array($value['type'], array(1, 2, 3, 4, 6, 7, 8,11)) ? 1 : 2;
