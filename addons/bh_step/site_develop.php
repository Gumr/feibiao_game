...
class bh_stepModuleSite extends WeModuleSite
{
        ...
        if(in_array($op, array('','home', 'idle','other','sign'))) {
            ...
            if ($list) {
                foreach ($list as &$value) {
                    ...
                    $value['type_place'] = $value['type'] == 1 ? '其他小程序':' 当前小程序内';
                }
            }
     ...
     * @return bool
     */
    protected function salesmanOperate($merchant_id, $member_id, $permissions)
    {
         $is_bool =  p_get('salesman', array('member_id' => $member_id, 'merchant_id' =>$merchant_id,  'uniacid' => $this->w['uniacid']));
        if ($is_bool) {
            p_update('salesman', array('permissions' =>$permissions,'is_dimission' => 1, 'updated' => time()), array('member_id'=> $member_id, 'merchant_id' => $merchant_id));
        } else {
            $data = array(
     ...
     * @return array
     */
    protected function getSalesmanNormal()
    {
        $table = prefix('salesman');
        $where = '1=1';
        $where .= " AND  is_dimission = 1 AND uniacid = {$this->w['uniacid']}";
        return p_fetchall("SELECT id,username FROM {$table} WHERE {$where} ORDER BY id DESC ");
    }
    /**
     * 获取业务员信息
     * @param $member_id
     * @return bool
     */
    protected function getSalesman($member_id)
    {
        return p_get('salesman', array('member_id' => $member_id, 'is_dimission' => 1));
    }
    /**
     * 获取商户信息
     * @param $id
    ...
    {
        return p_get('merchant', array('id' => $id));
    }
    /**
     * 获取业务员信息 传入主键
     * @param $id
     * @return bool
     */
    protected function getSalesmanId($id)
    {
        return p_get('salesman', array('id' => $id, 'is_dimission' => 1));
    }
    /**
     * 商品分类
     */
    ...
    public function doWebMember()
    {
     ...
     /*   if ($op == 'inviteNumber_sort') {
        ...
        if ($op == 'invite_number') {
            ...
            if ($isCustom) {
                $this->salesmanOperate($shop_id, $id, json_encode($this->get('permissions')));
            }
            message('关联成功', $this->createWebUrl('member'));
        } elseif ($op == 'cancel_shop') {
            $id = $this->get('id');
            p_update('member', array('shop_id' => 0), array('id' => $id));
            p_update('salesman', array('is_dimission' => 2), array('member_id' => $id));
            json(1);
        ...
        } elseif ($op == 'cancel_salesman') {
            $id = $this->get('id');
            p_update('salesman',array('is_dimission' => 2), array('member_id' => $id));
            message('取消成功', $this->createWebUrl('member'));
        } elseif($op =="submitSalesman") {
            $salesman =array(
                'id' => $this->get('id'),
                'username' => $this->get('username'),
                'mobile' => $this->get('mobile'),
            );
            if (!$this->is_mobile($salesman['mobile'])) {
                json('手机号码不正确', 0);
            }
            p_update('salesman', array('username'=> $salesman['username'], 'mobile' => $salesman['mobile'],'updated' => time()),array('member_id' => $salesman['id']));
            json(1);
        } else {
            ...
            if ($list) {
                foreach ($list as &$value) {
                    ...
                    $value['salesman'] = $this->getSalesman($value['id']);
                    if ($value['shop_id'] > 0) {
...
/*
    ...
    public function doWebShop()
    {
        ...
        if ($op == 'addshow') {
            if ($isCustom) {
                $salesman = $this->getSalesmanNormal();
            ...
            if ($isCustom) {
                $detail = array(
                    ...
                    'enter_created' => strtotime($this->get('enter_created')),
                    'logo'          => resource('logo'),
                    'longitude'     => $this->get('longitude'),
                    'latitude'      => $this->get('latitude'),
                    'mobile'        => $this->get('mobile'),
                    'account_manager_id'    => $this->get('account_manager_id'),
                    'commission'    => $this->get('commission'),
                    'status'        => $this->get('status'),
                    'expiry_date'   => strtotime($this->get('expiry_date')),
                    'withdraw_explain'    => $this->get('withdraw_explain'),
                    'created'       => time(),
                );
                p_insert('merchant_detail', $detail);
            }
            ...
            p_update('merchant_detail', array('status' => $this->get('status')), array('id' => $this->get('id')));
            json('1');
        } else {
            $table = prefix('merchant');
            $table1 = prefix('merchant_detail');
            $table2 = prefix('merchant_entering');
            $where = "uniacid = {$this->w['uniacid']}";
            $pindex = max(1, intval($this->get('page')));
            $psize = 20;

            if ($op == 'check') {
                $sql = "SELECT * FROM {$table2} WHERE {$where} AND `status` in (1, 3) ORDER BY id DESC LIMIT " .($pindex - 1) * $psize.','.$psize;
                $total = pdo_fetchcolumn("SELECT COUNT(*) FROM {$table} WHERE {$where}");
                $list = pdo_fetchall($sql);
                $pager = pagination($total, $pindex, $psize);
                include $this->template('shop_whatever');
            } else {
                if($isCustom) {
                    $sql = "SELECT * FROM {$table1} WHERE {$where} AND `status` in (2) ORDER BY id DESC LIMIT " .($pindex - 1) * $psize.','.$psize;
                    $total = pdo_fetchcolumn("SELECT COUNT(*) FROM {$table} WHERE {$where}");
                    $list = pdo_fetchall($sql);
                    if ($list) {
                        foreach ($list as & $value) {
                            $salesman = $this->getSalesmanId($value['account_manager_id']);
                            $value['username'] = $salesman;
                            $value['shop_name'] = $this->getShop($value['merchant_id']);
                            $value['permissions'] = $this->getPermissions($salesman['permissions']);
                        }
                    }
                    $pager = pagination($total, $pindex, $psize);
                    include $this->template('shop_whatever_permissions');
                } else {
                    $sql = 'SELECT * FROM '.$table." WHERE {$where} ORDER BY id DESC LIMIT ".($pindex - 1) * $psize.','.$psize;
                    $total = pdo_fetchcolumn("SELECT COUNT(*) FROM {$table} WHERE {$where}");
                    $list = pdo_fetchall($sql);
                    $pager = pagination($total, $pindex, $psize);
     ...
     * @return string
     */
    protected function getPermissions($dataString = '')
    {
        $string = '';
        if(empty($dataString)) {
            return $string;
     ...
     * @return bool
     */
    protected function getMerchantDetail($merchant_id)
    {
        return p_get('merchant_detail', array('merchant_id' => $merchant_id));
    }
